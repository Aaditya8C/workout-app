...
const TemplatesHomePage = () => {
  ...
  return (
    <SafeAreaView className="flex-1 px-1 bg-violet-200 ">
      <View className="flex-1 flex flex-col gap-2">
        <View className="pt-2 flex flex-row  justify-center  items-center">
          <View className="flex flex-row items-center rounded-lg  p-1">
            ...
            ></Image>
            <Text className="font-bold tracking-wider  text-2xl">STRETCH</Text>
          </View>
        </View>
        <View className="w-[90%] self-center">
          <Pressable
            className="rounded-lg bg-violet-950 p-3 items-center flex flex-row justify-center"
            style={{ elevation: 5 }}
            onPress={() => {
              router.push("/createTemplate");
            }}
          >
            <Text className="text-white  font-bold text-sm">ADD TEMPLATE </Text>

            <Icon name="add" size={23} color="white"></Icon>
          </Pressable>
        </View>
        <View>
          <Text className="pl-4 tracking-widest ">MY TEMPLATES</Text>
        </View>
        <MenuProvider>
          <FlatList
            data={templates}
